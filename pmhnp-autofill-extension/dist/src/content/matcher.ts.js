import { US_STATES, STATE_ABBREVIATION_TO_NAME } from "/src/shared/constants.ts.js";
export function mapFieldsToProfile(fields, profile) {
  return fields.map((field) => mapSingleField(field, profile)).filter(Boolean);
}
function mapSingleField(field, profile) {
  ...
  const status = value === "" || value === null || value === void 0 ? "no_data" : mapping.confidence < 0.5 ? "ambiguous" : "ready";
  return {
    field,
...
function getDirectMapping(field, profile) {
  ...
  const map = {
    ...
    license_number: () => {
      ...
      return license ? { profileKey: "credentials.licenses[0].licenseNumber", value: license.licenseNumber, confidence: field.confidence } : { profileKey: "credentials.licenses[0].licenseNumber", value: "", confidence: 0.3 };
    },
    license_state: () => {
      const license = findBestLicenseForContext(c.licenses, {});
      return license ? { profileKey: "credentials.licenses[0].licenseState", value: license.licenseState, confidence: field.confidence } : null;
    },
    license_expiration: () => {
      const license = findBestLicenseForContext(c.licenses, {});
      return license ? { profileKey: "credentials.licenses[0].expirationDate", value: formatDate(license.expirationDate), confidence: field.confidence } : null;
    },
    // Certification
    certification_number: () => {
      const cert = c.certifications[0];
      return cert ? { profileKey: "credentials.certifications[0].certificationNumber", value: cert.certificationNumber || "", confidence: field.confidence } : null;
    },
    // Education
    degree: () => {
      const edu = profile.education.find((e2) => e2.isHighestDegree) || profile.education[0];
      return edu ? { profileKey: "education[0].degreeType", value: edu.degreeType, confidence: field.confidence } : null;
    },
    school: () => {
      const edu = profile.education.find((e2) => e2.isHighestDegree) || profile.education[0];
      return edu ? { profileKey: "education[0].schoolName", value: edu.schoolName, confidence: field.confidence } : null;
    },
    graduation_date: () => {
      const edu = profile.education.find((e2) => e2.isHighestDegree) || profile.education[0];
      return edu ? { profileKey: "education[0].graduationDate", value: formatDate(edu.graduationDate), confidence: field.confidence } : null;
    },
    field_of_study: () => {
      const edu = profile.education.find((e2) => e2.isHighestDegree) || profile.education[0];
      return edu ? { profileKey: "education[0].fieldOfStudy", value: edu.fieldOfStudy || "", confidence: field.confidence } : null;
    },
    gpa: () => {
      const edu = profile.education.find((e2) => e2.isHighestDegree) || profile.education[0];
...
function mapScreeningAnswer(field, profile, key) {
...
function mapFileField(field, profile) {
...
export function findBestLicenseForContext(licenses, context) {
  if (licenses.length === 0) return null;
  let candidates = [...licenses];
  if (context.state) {
    const stateMatch = candidates.filter(
      (l) => l.licenseState.toLowerCase() === context.state.toLowerCase() || l.licenseState === (US_STATES[context.state] || context.state) || l.licenseState === (STATE_ABBREVIATION_TO_NAME[context.state.toUpperCase()] || context.state)
    );
...
export function matchDropdownOption(profileValue, options) {
...
export function fuzzyMatch(text1, text2) {
...
function levenshtein(s1, s2) {
  ...
  const dp = Array.from(
    { length: len1 + 1 },
    (_, i) => Array.from({ length: len2 + 1 }, (_2, j) => i === 0 ? j : j === 0 ? i : 0)
  );

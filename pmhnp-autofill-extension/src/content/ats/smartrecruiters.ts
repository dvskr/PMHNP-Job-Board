...
export async function runFullSmartRecruitersFlow(profile: Record<string, unknown>): Promise<void> {
...
async function handleSection(name: string, profile: Record<string, unknown>): Promise<void> {
    ...
    if (!heading) { console.log(`[PMHNP] ❌ No "${name}" heading`); return; }
    ...
    if (!btn) { console.log(`[PMHNP] ❌ No Add button for "${name}"`); return; }
    console.log(`[PMHNP] ✅ Clicking Add: "${btn.textContent?.trim()}"`);
    btn.click(); // Only ONE click — dispatchEvent caused double-entry
    await sleep(1500);
...
// ──────────────────────────────────────────────────
// FILL EXPERIENCE — known layout order
...
async function fillExpFields(rows: HTMLElement[][], profile: Record<string, unknown>): Promise<void> {
...
// ──────────────────────────────────────────────────
// FILL EDUCATION — known layout order
...
async function fillEduFields(rows: HTMLElement[][], profile: Record<string, unknown>): Promise<void> {
    const edu = (profile as any)?.education?.[0];
    if (!edu) { console.log('[PMHNP] No education data'); return; }
...
// ──────────────────────────────────────────────────
// AUTOCOMPLETE — type char by char then click suggestion
// ──────────────────────────────────────────────────
...
async function fillAutocomplete(input: HTMLInputElement, value: string): Promise<void> {
    ...
    // Search for suggestion options — only click if the suggestion text matches our value
    const selectors = [
    ...
    for (const sel of selectors) {
        ...
        if (visible.length > 0) {
            ...
            if (bestMatch) {
                ...
                // Suggestions exist but none match — click FIRST one as it's likely the closest
    ...
    // No suggestions at all — just Tab/Blur to accept the typed value
...
async function fillDate(input: HTMLInputElement, dateStr: string): Promise<void> {
    ...
    try {
        ...
        // Use UTC methods to avoid timezone shift (e.g., 2022-03-01 → Feb 28 in CST)
...
async function clickCheckboxByText(text: string): Promise<void> {
    ...
    for (const label of labels) {
        ...
        if (cb && !cb.checked) { cb.click(); cb.dispatchEvent(new Event('change', { bubbles: true })); console.log(`[PMHNP] ✅ Checked "${text}"`); return; }
    }
    ...
    for (const label of labels) {
        ...
        for (const cb of cbs) {
            if (Math.abs(cb.getBoundingClientRect().top - lR.top) < 30 && !cb.checked) {
                cb.click(); cb.dispatchEvent(new Event('change', { bubbles: true })); console.log(`[PMHNP] ✅ Checked "${text}" (proximity)`); return;
            }
        }
    }
}

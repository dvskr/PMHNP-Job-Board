...
if (Test-Path ".env.local") {
    Get-Content ".env.local" | ForEach-Object {
        ...
        if ($_ -match "^JOOBLE_API_KEY=(.+)$") {
            $env:JOOBLE_API_KEY = $matches[1]
        }
    }
}

Write-Host "Testing Jooble API Integration..." -ForegroundColor Cyan
Write-Host ""
...
if (-not $env:JOOBLE_API_KEY) {
    Write-Host "WARNING: JOOBLE_API_KEY not set in .env.local" -ForegroundColor Yellow
}

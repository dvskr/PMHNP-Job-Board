...
> **Key Takeaway:** Oklahoma is **transitioning to independent practice** via HB 2298 (effective Nov 2025). After **6,240 supervised hours**, PMHNPs can prescribe independently, but **Schedule II drugs (including ADHD stimulants) are completely prohibited** for APRNs. HRSA's designated Mental Health HPSA data puts Oklahoma at only **21.4%** of needs met across **124 HPSAs**, and a state loan repayment program runs alongside the federal NHSC awards. See [current advertised pay for PMHNPs in Oklahoma](/salary-guide/oklahoma).

The landscape for Psychiatric-Mental Health Nurse Practitioners (PMHNPs) in Oklahoma has officially entered a new era. HRSA's designated Mental Health HPSA data puts the state at only **21.39%** of its mental healthcare needs met. Driven by that shortage, Oklahoma enacted landmark legislation (HB 2298) effective late 2025, granting experienced APRNs the right to apply for independent prescriptive authority.
...
| **APRN Compact?** | ❌ No |
| **Advertised PMHNP Pay** | [Current advertised pay in Oklahoma](/salary-guide/oklahoma) |
| **Prescriptive Authority** | Yes, independent path available (Schedule II **prohibited**; Schedule III-V only, 30-day supply max) |

*Licence and application fees above are set by the Oklahoma Board of Nursing and change periodically: verify the current amounts with the Board before you apply.*
...
Maintain current, active national certification in your psychiatric-mental health specialty. The ANCC sets the exam fee and adjusts it periodically, so verify the current amount with the ANCC before you register.
...
| Application fee | **$85**, set by the Oklahoma Board of Nursing, paid via Nurse Portal |
...
1. **Federal DEA registration**. The DEA sets this fee (roughly $888 for a three-year registration); verify the current amount with the DEA before you apply.
...
Oklahoma is an NLC member. However, the **APRN Compact is not enacted**, so out-of-state PMHNPs must hold an active **Oklahoma APRN license** and apply for prescriptive authority.
...
Oklahoma's behavioral health shortage keeps competition for psychiatric specialists strong, and psychiatric roles generally advertise above the range seen for nurse practitioners overall. Instead of a headline figure we cannot trace to a source, we publish what Oklahoma employers are advertising right now: see [current advertised pay for PMHNPs in Oklahoma](/salary-guide/oklahoma), drawn from live listings on this site.
...
Practicing in Oklahoma's shortage areas unlocks loan forgiveness. The federal maximums below are set by HRSA's National Health Service Corps and the state award is set by the Oklahoma State Department of Health; verify the current amount with the awarding body before you apply:

| Program | Award Amount | Service Requirement |
|:---|:---|:---|
| **Oklahoma State Health Department SLRP** | Set by the state program each funding cycle | Typically 2 years in underserved area |
...
5. **Out-of-state hours count:** If you've accumulated supervised prescribing hours in another state, those **count toward your 6,240-hour requirement**, so you don't need to start from zero.
...
A: Yes. Oklahoma is an active NLC member. However, the APRN Compact is not enacted, so you need an Oklahoma-specific APRN credential for advanced practice.

...
const entrylevelFaqs = [
  { question: 'What is a Entry Level PMHNP role?', answer: 'Entry Level PMHNP positions focus on psychiatric mental health care in entry level settings, offering specialized clinical opportunities.' },
  { question: 'What does a Entry Level PMHNP earn?', answer: 'Entry Level PMHNPs typically earn $100K-140K annually depending on location, experience, and setting.' },
  ...
  { question: 'How do I find entry level positions?', answer: 'Browse our curated entry level job listings above, set up alerts, and apply directly through our platform.' },
];

export async function generateMetadata(): Promise<Metadata> {
  const stats = await getStats();
  return {
    title: `${stats.totalJobs} Entry Level PMHNP Jobs ($100K-140K)`,
    description: `Find ${stats.totalJobs} entry level PMHNP jobs paying $100K-140K+. Browse psychiatric nurse practitioner positions.`,
    alternates: { canonical: 'https://pmhnphiring.com/jobs/entry-level' },
  };
}
  ...
  return (
    <div style={{ backgroundColor: '#FDFBF7' }}>
      <BreadcrumbSchema items={[
        ...
        { name: "Entry Level", url: "https://pmhnphiring.com/jobs/entry-level" }
      ]} />
      <JobListViewTracker jobs={jobs.map((j: Job) => ({ id: j.id, title: j.title, employer: j.employer }))} listName="Entry Level Jobs" />

      {/* ═══ HERO ═══ */}
      <section style={{ background: '#d6e5c8', padding: '72px 0 56px' }}>
        <div style={{ maxWidth: '1140px', margin: '0 auto', padding: '0 24px' }}>
          <div className="cat-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', alignItems: 'center' }}>
            <div>
              ...
              <h1 className="font-lora" style={{ fontSize: 'clamp(32px, 4.2vw, 48px)', fontWeight: 800, lineHeight: 1.08, color: '#1A2E35', margin: '0 0 20px' }}>Entry Level<br /><span style={{ color: '#0D9488' }}>PMHNP Jobs</span></h1>
              <p style={{ fontSize: '16px', color: '#3D2E26', lineHeight: 1.7, margin: '0 0 36px', maxWidth: '440px' }}>Browse entry level psychiatric nurse practitioner positions with competitive pay and benefits.</p>
              <Link href="/jobs?q=entry+level" className="cat-cta-primary" style={{ padding: '16px 40px', borderRadius: '16px', fontWeight: 700, fontSize: '15px', background: '#0D9488', color: '#fff', textDecoration: 'none', display: 'inline-flex', alignItems: 'center', gap: '10px', boxShadow: '4px 4px 14px rgba(13,148,136,0.25)' }}>Browse Entry Level Jobs <ArrowRight size={17} /></Link>
            </div>
      ...
      <div style={{ background: 'linear-gradient(180deg, #F0FDFA 0%, #E6FAF5 50%, #F0FDFA 100%)' }}>
        <section style={{ maxWidth: '1000px', margin: '0 auto', padding: '48px 20px 40px' }}>
          ...
          <h2 className="font-lora" style={{ fontSize: 'clamp(26px, 3.5vw, 38px)', fontWeight: 700, color: '#1A2E35', textAlign: 'center', marginBottom: '48px' }}>Built for Entry Level Care</h2>
          <div className="cat-bento-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(12, 1fr)', gap: '14px' }}>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 6', padding: '28px 24px' }}>
              ...
              <p style={{ fontSize: '13px', color: '#5A4A42', lineHeight: 1.6, margin: 0 }}>Focus on entry level psychiatric care with dedicated clinical environments.</p>
            </div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 6', padding: '28px 24px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#1A2E35', margin: '0 0 8px' }}>Competitive Pay</h3>
              <p style={{ fontSize: '13px', color: '#5A4A42', lineHeight: 1.6, margin: 0 }}>Entry Level PMHNPs earn $100K-140K with benefits, loan repayment, and bonuses.</p>
            </div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 6', padding: '28px 24px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#1A2E35', margin: '0 0 8px' }}>Growing Demand</h3>
              <p style={{ fontSize: '13px', color: '#5A4A42', lineHeight: 1.6, margin: 0 }}>High demand for entry level PMHNPs nationwide with strong job security.</p>
            </div>
            <div className="cat-bento-card" style={{ ...clayCard, gridColumn: 'span 6', padding: '28px 24px' }}>
              <h3 style={{ fontSize: '16px', fontWeight: 800, color: '#1A2E35', margin: '0 0 8px' }}>Career Growth</h3>
              <p style={{ fontSize: '13px', color: '#5A4A42', lineHeight: 1.6, margin: 0 }}>Advance from clinical roles to leadership positions in entry level settings.</p>
            </div>
          </div>
        </section>
      </div>

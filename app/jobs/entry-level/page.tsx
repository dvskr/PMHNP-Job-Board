...
const EL_FILTER = { isPublished: true, OR: CATEGORY_FILTERS['entry-level'], AND: [...GLOBAL_EXCLUSIONS.map(e => ({ NOT: e })), ...(CATEGORY_EXCLUSIONS['entry-level'] || []).map((e: any) => ({ NOT: e }))] };
...
export default async function EntryLevelPage({ searchParams }: PageProps) {
  ...
  return (
    <div style={{ backgroundColor: '#FDFBF7' }}>
      ...
      {/* HERO */}
      <section style={{ background: '#cbd9b8', padding: '72px 0 56px' }}>
        <div style={{ maxWidth: '1140px', margin: '0 auto', padding: '0 24px' }}>
          <div className="cat-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', alignItems: 'center' }}>
            <div>
              ...
              <p style={{ fontSize: '16px', color: '#3D2E26', lineHeight: 1.7, margin: '0 0 36px', maxWidth: '440px' }}>New grad friendly positions with mentorship, structured onboarding, and growing caseloads.</p>
            ...
            <div style={{ display: 'flex', justifyContent: 'center' }}>
              <Image src="/images/categories/hero_wc_entrylevel.png" alt="New grad PMHNP starting career" width={520} height={520} style={{ width: '100%', maxWidth: '500px', height: 'auto' }} priority />
            </div>
          </div>
        </div>
      </section>
      ...
      <div style={{ background: 'linear-gradient(180deg, #F0FDFA 0%, #E6FAF5 50%, #F0FDFA 100%)' }}>
        <section style={{ maxWidth: '1000px', margin: '0 auto', padding: '56px 20px' }}>
          ...
          <div className="cat-explore-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '14px' }}>
            {[{ href: '/jobs/new-grad', label: 'New Grad', sub: 'First-time roles' }, { href: '/jobs/remote', label: 'Remote', sub: 'Work from home' }, { href: '/jobs/telehealth', label: 'Telehealth', sub: 'Virtual care' }, { href: '/jobs/outpatient', label: 'Outpatient', sub: 'Clinic-based' }, { href: '/salary-guide', label: 'Salary Guide', sub: '2026 data' }, { href: '/jobs/locations', label: 'By Location', sub: '50 states' }].map(c => (

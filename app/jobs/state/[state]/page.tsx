      ...
      {jobs.length > 0 && (
        <script type="application/ld+json" dangerouslySetInnerHTML={{ __html: JSON.stringify(
          jobs.slice(0, 10).map((job: Job) => ({
            ...
            '@type': 'JobPosting',
            title: job.title,
            description: (job.descriptionSummary || job.description || '').slice(0, 500) || `PMHNP position in ${stateName}`,
            ...
            employmentType: job.jobType === 'Part-Time' ? 'PART_TIME' : job.jobType === 'Contract' ? 'CONTRACTOR' : job.jobType === 'Per Diem' ? 'PER_DIEM' : 'FULL_TIME',
            hiringOrganization: { '@type': 'Organization', name: job.employer || 'Confidential Employer', ...(job.companyLogoUrl && { logo: job.companyLogoUrl }) },
            jobLocation: { '@type': 'Place', address: { '@type': 'PostalAddress', ...(job.city && { addressLocality: job.city }), addressRegion: job.stateCode || stateCode || '', addressCountry: 'US' } },

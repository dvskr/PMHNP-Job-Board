...
const seniorFaqs = [
  { question: 'What is a Senior PMHNP role?', answer: 'Senior PMHNP positions focus on psychiatric mental health care in senior settings, offering specialized clinical opportunities.' },
  { question: 'What does a Senior PMHNP earn?', answer: 'Senior PMHNPs typically earn $160K-220K annually depending on location, experience, and setting.' },
  ...
  { question: 'How do I find senior positions?', answer: 'Browse our curated senior job listings above, set up alerts, and apply directly through our platform.' },
];

export async function generateMetadata(): Promise<Metadata> {
  const stats = await getStats();
  return {
    title: `${stats.totalJobs} Senior PMHNP Jobs ($160K-220K)`,
    description: `Find ${stats.totalJobs} senior PMHNP jobs paying $160K-220K+. Browse psychiatric nurse practitioner positions.`,
...
export default async function SeniorPage({ searchParams }: PageProps) {
  ...
  return (
    <div style={{ backgroundColor: '#FDFBF7' }}>
      ...
      {/* ═══ HERO ═══ */}
      <section style={{ background: '#c8d4e5', padding: '72px 0 56px' }}>
        <div style={{ maxWidth: '1140px', margin: '0 auto', padding: '0 24px' }}>
          <div className="cat-hero-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '40px', alignItems: 'center' }}>
            <div>
              ...
              <h1 className="font-lora" style={{ fontSize: 'clamp(32px, 4.2vw, 48px)', fontWeight: 800, lineHeight: 1.08, color: '#1A2E35', margin: '0 0 20px' }}>Senior<br /><span style={{ color: '#0D9488' }}>PMHNP Jobs</span></h1>
              <p style={{ fontSize: '16px', color: '#3D2E26', lineHeight: 1.7, margin: '0 0 36px', maxWidth: '440px' }}>Browse senior psychiatric nurse practitioner positions with competitive pay and benefits.</p>

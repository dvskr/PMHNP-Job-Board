...
/**
 * CCPA / CPRA "Do Not Sell or Share" interactive body.
 *
 * Clicking opt-out:
 *   1. Calls denyAllConsent() (resets GA Consent Mode v2 signals)
 *   2. Persists ALL_DENIED categories in the HttpOnly consent cookie so the
 ...
 *
 * `initialOptedOut` is resolved by the server shell, the only side that can
 * read the HttpOnly consent cookie. It is what makes the confirmation survive
 * a reload rather than the page offering the button again as if nothing had
 * been recorded.
 */
export default function DoNotSellClient({ initialOptedOut }: { initialOptedOut: boolean }) {

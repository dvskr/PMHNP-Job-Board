  ...
  return (
    ...
    >
      {shareLinks.map((link) => (
        <button
          key={link.name}
          onClick={() => openShareWindow(link.href)}
          className={`w-full px-4 py-2 text-left text-sm text-gray-700 hover:bg-gray-100 flex items-center gap-3 transition-colors ${link.hoverClass}`}
          role="menuitem"
        >

...
function JobCard({ job, viewMode = 'grid' }: JobCardProps) {
  ...
  const shortLocation = (() => {
    ...
    return first.length > 35 ? first.slice(0, 33) + 'â€¦' : first;
  })();
  ...
  return (
    <Link href={jobUrl} className="block touch-manipulation h-full" onClick={handleCardClick}>
      ...
      >
        ...
        {/* Row 2: Salary â€” inline, not full width */}
        {salaryDisplay && (
          <div>
            <Badge variant="salary" size="sm">
              {salaryDisplay.startsWith('$') ? salaryDisplay : `$${salaryDisplay}`}
        ...
        {/* Row 6: Footer â€” Date + CTA Button */}
        <div style={{
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
            ...
            >
              View Job <span className="jc-cta-arrow" style={{ transition: 'transform 0.2s ease' }}>â†’</span>
            </span>
            {job.applyLink && (
              <button
                className="jc-apply-btn"

...
export default function ApplyButton({ jobId, applyLink, jobTitle, isAuthenticated = false, applyOnPlatform = false }: ApplyButtonProps) {
  ...
  const handleApply = () => {
    ...
    if (applyLink) {
      // Track apply click (fire and forget â€” internal analytics)
      try {
  ...
  return (
    <div className="flex flex-col w-full">
      ...
      {showPlatformApply ? (
        ...
        /* Inline Auth Gate â€” replaces button area when triggered */
        <div className="w-full">
          ...
          <div className="space-y-2 mb-4">
            {[
              { icon: 'ðŸ‘€', text: 'Get noticed by employers hiring PMHNPs' },
              { icon: 'ðŸ’¬', text: 'Receive direct messages from recruiters' },
              { icon: 'âš¡', text: 'Auto-fill applications with our Chrome extension (Coming soon)' },
            ].map((item) => (

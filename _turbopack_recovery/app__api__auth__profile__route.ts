  ...
  try {
    ...
    if (!profile && user.email) {
      ...
      if (profile && profile.supabaseId !== user.id) {
        ...
        // No profile exists at all â€” create one
...
export async function POST(request: NextRequest) {
  try {
    ...
    // Create leads for new signups â€” job seekers go to email_leads, employers go to employer_leads
    // IMPORTANT: EmailLead must be created BEFORE JobAlert (foreign key: JobAlert.email â†’ EmailLead.email)
    if (!existingProfile) {
      try {
        if (role === 'employer') {

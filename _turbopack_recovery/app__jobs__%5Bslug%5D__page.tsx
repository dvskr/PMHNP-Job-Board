...
// connection pool â†’ 5xx errors. 3600s trades freshness for stability.
...
const getJob = cache(async function getJob(id: string): Promise<JobResult> {
  try {
    ...
    if (!anyJob) {
      // Job was deleted from DB entirely â€” return 410 Gone so Google stops recrawling
      return { status: 'gone' };
    }

    // Job exists but is unpublished/expired â†’ 410 Gone
    if (!anyJob.isPublished) {
      return { status: 'expired', employer: anyJob.employer, title: anyJob.title };
    }

    // Job is published â€” fetch full data with employer info
...
async function getCompanyInfo(companyId: string | null, employerName: string, jobId?: string) {
  ...
  // No Company record found â€” try to build one from EmployerJob data
  if (jobId) {
    const ej = await prisma.employerJob.findUnique({
      where: { jobId },
...
export async function generateMetadata({ params }: JobPageProps) {
  ...
  if (!id) {
    // Malformed slug with no UUID â€” return 410 to stop Google recrawling
    return {
  ...
  // Job completely deleted from DB â†’ 410 Gone
  ...
  // GSC Fix: Expired jobs â†’ noindex page with rich content instead of bare 404.
  ...
  if (result.status === 'expired') {
    ...
    return {
      title: `${expiredTitle} â€” Position Filled`,
    ...
    <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ backgroundColor: 'var(--bg-primary)' }}>
      <div className="max-w-2xl w-full text-center">
        ...
        <p className="text-base mb-8" style={{ color: 'var(--text-tertiary)' }}>
          Don&apos;t worry â€” we have hundreds of similar PMHNP positions available right now.
        </p>
...
export default async function JobPage({ params }: JobPageProps) {
  ...
  // Job completely deleted â†’ render 410 Gone page
  if (result.status === 'gone') {
    return renderGonePage();
  }

  // Job exists but expired/unpublished â†’ render rich expired page
  // (not notFound() â€” see generateMetadata for rationale)
  if (result.status === 'expired') {
    const expiredTitle = result.title || 'PMHNP Position';
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-4 py-16" style={{ backgroundColor: 'var(--bg-primary)' }}>
        <div className="max-w-2xl w-full text-center">
          ...
          <p className="text-base mb-8" style={{ color: 'var(--text-tertiary)' }}>
            This job has been filled or the listing has expired. Don&apos;t worry â€” we have hundreds of similar PMHNP positions available.
          </p>
        ...
        <div className="lg:grid lg:grid-cols-[1fr_340px] lg:gap-8">
              ...
              <div className="rounded-2xl overflow-hidden mb-5 lg:mb-6" style={{ backgroundColor: '#F7FBF8', border: '1px solid rgba(0,0,0,0.06)', borderRadius: '20px', boxShadow: '6px 6px 12px rgba(0,0,0,0.06), -2px -2px 8px rgba(255,255,255,0.8), inset 1px 1px 2px rgba(255,255,255,0.6)', position: 'relative', padding: '24px 24px 28px', }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '12px', flexWrap: 'wrap' }}>
                  ...
                  <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                    ...
                    <span style={{ color: 'var(--text-tertiary)', fontSize: '18px', lineHeight: 1 }}>Â·</span>
                      ...
                      if (paragraph.trim().startsWith('â€¢')) {
                        return (
                          <div key={index} className="flex items-start gap-2 ml-4 my-1">
                            <span className="mt-1 font-bold" style={{ color: 'var(--color-primary)' }}>â€¢</span>
          ...
          <AnimatedContainer animation="slide-in-right" delay={300}>
            <div className="mt-6 lg:mt-0">
                ...
                {!expiryStatus.isExpired && expiryStatus.text && (
                  <div className={`flex items-center gap-2 mb-4 pb-3 ${expiryStatus.isUrgent ? 'text-orange-500' : ''}`} style={{ borderBottom: '1px solid var(--border-color)', color: expiryStatus.isUrgent ? undefined : 'var(--text-tertiary)' }}>
                    ...
                    <p className="text-sm font-medium">
                      {expiryStatus.text}{expiryStatus.isUrgent && ' â€” Apply soon!'}
                    </p>
                  </div>
                )}
              ...
              {/* About Employer â€” separate card */}
              ...
              {/* Career Resources â€” separate card */}

...
const TAXONOMY_LEADS: Record<string, TaxonomyLeadFn> = {
    ...
    'contract': (f) => `Contract PMHNP positions in ${f.city.name} are typically 1099 engagements paid hourly rather than salaried. Contractors handle their own self-employment tax, malpractice insurance, and quarterly estimated payments, but they retain higher take-home than equivalent W-2 roles. ${f.shortage ? 'Shortage-area employers here lean on contract coverage to hold clinics open while permanent searches run.' : `Contract demand in ${METRO_SCALE_PHRASES[f.populationTier]} tracks vacancy cycles at the larger systems.`}`,
    ...
    'per-diem': (f) => `Per-diem PMHNP shifts in ${f.city.name} are booked hourly with no scheduled commitment, which is why the rate sits above the hourly equivalent of a salaried role. ${isExpensive(f.costTier) ? `With ${COST_PHRASES[f.costTier]}, local per-diem rates run near the top of the market and most providers stack shifts alongside a primary employer.` : `With ${COST_PHRASES[f.costTier]}, local per-diem rates sit lower but go further against local expenses.`}`,

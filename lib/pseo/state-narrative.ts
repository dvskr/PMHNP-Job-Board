...
const SETTING_LEADS: Record<string, SettingLeadFn> = {
    'remote': (c) => isNlcMember(c.stateName)
        ? `Remote PMHNP positions covering patients in ${c.stateName} typically pay $130K to $200K+ and require active ${c.stateCode} state licensure plus a HIPAA-compliant home setup. Most postings welcome multi-state Compact licensure to broaden caseload reach.`
        : `Remote PMHNP positions covering patients in ${c.stateName} typically pay $130K to $200K+ and require active ${c.stateCode} state licensure plus a HIPAA-compliant home setup. ${c.stateName} is not a Nurse Licensure Compact member, so a separate ${c.stateCode} license is required even if you already hold a multistate license elsewhere.`,

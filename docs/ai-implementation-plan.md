...
| 1.1 — Smart job matching | ✅ Done (2026-05-02, refined 2026-05-03) | `/api/jobs/search/semantic` route (vector + keyword RRF, degraded fallback). `/jobs/search` page + `<SemanticJobSearch>` client component — refactored to use the existing clay design tokens (FDFBF7 page bg, white clay cards, F5F0EB recessed inputs) and the existing `<JobCard>` component for results. % match badge overlays the card top-right. Behind `ai.search.semantic` flag. Seed `job-search.json` golden set + runbook `ai-job-search.md`. |
